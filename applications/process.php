...
if( $_POST['action'] == 'delete_form'){
    $id = $_POST['id'];
    $db_error = '';
    $query = $conn->query("DELETE FROM `mmacayly_data`.`applications` WHERE `id` = '".$id."'");
    $db_error .= $conn->error;
    if($query === TRUE){
        $msg = 'Form successfully removed!';
    }else{
        $msg = $db_error;
    }
        die(json_encode(['msg' => $msg.$id]));
}
    ...
    for($i = 1; $i <= $num_siblings; $i++){
        $html .= "<li id='field_2_45' class='gfield gf_left_half field_sublabel_below field_description_above gfield_visibility_visible' >
                    ...
                    <div class='ginput_complex ginput_container no_prefix has_first_name no_middle_name has_last_name no_suffix gf_name_has_2 ginput_container_name gfield_trigger_change' id='input_2_45'>
                        ...
                        <input type='text' name='sibling_".$i."_lastname' id='input_2_45_6' value='' aria-label='Last name' tabindex='120'   aria-invalid='false' />
...
$html = '<html>
        ...
        <body>
            <div style="width:100%;height:190px;background: #475675">
                <img class="vc_single_image-img logo" src="../images/basic/logo-mm-full-new.png" alt="MM badge" title="MM badge" style="width: 100%;margin: 0 auto 0 auto; display: block; padding-top: 28px;">
...
$dompdf->stream(strtolower($pdf_name));
